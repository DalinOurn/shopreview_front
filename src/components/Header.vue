...
export default {
  ...
  watch: {
   async searchText(text) {
      console.log(text)
       ...
       const res = await fetch(`http://13.231.117.178:3001/store/all?name=${text}`, {
        method: 'GET',
        credentials: 'include',
        headers: {
  ...
  methods: {
    menuToggle() {
     ...
     async onLogout() {
      console.log('test')
      const res = await fetch('http://13.231.117.178:3001/auth/logout', {
        method: 'POST',
        credentials: 'include',
        headers: {
          "Content-type": "application/json",
    ...
    async getFavorite(){
      ...
      const get_favorites = await fetch(`http://13.231.117.178:3001/favorite/all?user_id=${userData._id}`, {
        method: 'GET',
        credentials: 'include',
        headers: {
          'Content-type': 'application/json',
