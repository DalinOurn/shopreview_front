    ...
    <div class="wrapper_browse">
      ...
      <div class="list_store">
        <div class="box" v-for="store in stores" :key="store._id">
          ...
          <router-link :to="`http://35.78.170.132:3001/store/${store._id}`">
            <div class="img">
              <img
                :src="store.imageUrl"
                alt="erorUserpost"
          ...
          <div class="des">
            ...
            <div>
              <router-link :to="`http://35.78.170.132:3001/store/${store._id}`">see more</router-link>
            </div>
          </div>
        </div>
      </div>
  ...
  methods: {
    ...
    async getStore(page, category) {
      ...
      const res = await fetch(
        `http://35.78.170.132:3001/store/all?page=${page}&category=${category}`,
        {
          method: 'GET',
          credentials: 'include',
          headers: {
